...
def load_data():
    # temp = arff.loadarff("95k-random.arff")
    file = pd.read_csv("95k-random.csv", header=None, skiprows=1)
    # file = pd.DataFrame(temp[0])
    label_col = file.shape[1] - 1
    label = file[[label_col]].values
    ...
    data = file.drop([label_col], axis=1).values
...
batch_size = 1000 # Here we applied the mini-batch gradient decsend
iterations = 3000
...
with tf.Graph().as_default() as graph:
    ...
    output_from_H1 = construct_layer(x, 12, 30, 'H1', activation=tf.nn.tanh)
    output_from_H2 = construct_layer(output_from_H1, 30, 20, 'H2', activation=tf.nn.tanh)
    output_from_H3 = construct_layer(output_from_H2, 20, 10, 'H3', activation=tf.nn.tanh)
    ...
    output_layer_logits = construct_layer(dropped, 10, 2, 'output', activation=None)
    ...
    with tf.name_scope('Loss'):
        adjust_weight = tf.constant([0.1, 100])
    ...
    with tf.name_scope('Train'):
        optimizer = tf.train.GradientDescentOptimizer(0.0003).minimize(loss)
...
with tf.Session(graph=graph) as sess:
    ...
    for i in range(iterations):
        ...
        if (i % 10 ==0):
            ...
            print('Validation accuracy at iteration %s: %s' %(i, acc))
            # if (acc > 0.90):
            #     break
        else:
